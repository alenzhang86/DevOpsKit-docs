...
### [Automatically Generating Fixes](Readme.md#automatically-generating-fixes-1)
...
the [Getting Started](../tree/master/00b-Getting-Started) document. It focuses on what you can do next after you have run the scans. 
...
This can ensure that you take the right next steps for a control in the context of a specific resource. 
...
> **Note**: Please use utmost discretion when attesting controls. In particular, when choosing not to fix a failing control, you are taking accountability that nothing will go wrong even though security is not correctly/fully configured. 
...
4. This is repeated for all the attestable controls and each resource.
...
|**StateConfirmed | User has verified the control data and attesting it as state confirmed to represent that the control state is correct/appropriate with proper justification. |
 ...
 >  ** These are special attestation statuses which are supported only for selected controls.
...
|Verify |NotAnIssue |Passed |Yes | 90 |User has to ratify based on manual examination that the finding does not apply as the control has been implemented in another way. For example, AAD authentication for App Service was implemented through code. |
...
|Failed |None |Failed |No | -NA- | Control has failed but has not been attested. Perhaps a fix is in the work...|	 
...
|Failed |NotApplicable |Passed |Yes| 90 |Control has failed. However, user confirms based on manual examination that the finding is not applicable for the given design/context.|
...
|Error |WillNotFix |Exception |Yes | Based on the control severity table below| There was an error during evaluation. Manual verification by the user indicates a valid security issue.|
|Error |WillFixLater |Remediate |Yes | Based on the control severity table below| There was an error during evaluation. Manual verification by the user indicates a valid security issue.|
|Error |NotApplicable |Passed |Yes| 90 |There was an error during evaluation. However, user confirms based on manual examination that the finding is not applicable for the given design/context.|
...
|Manual |NotApplicable |Passed |Yes| 90 |The control is not automated and has to be manually verified. User confirms based on manual examination that the finding for the given design/context is not applicable. |
...
> **Note**: The attestation data stored in the AzSKRG is opaque from the end user standpoint. Any attempts to access/change it may impact correctness of security evaluation results.  
...
To understand this better, let us take two scenarios where bulk attestation can help,
...
cannot afford to lose in an event of regional disasters. Thus, we have a situation where an AzSK control (in this case
the GRS setting for Storage Accounts) must pass for a subset of resources while it needs to be 'attested' for 
the remainder.
...
The above command will record the justification and attestation input/choice for all the specified Storage Accounts for the GRS control.
...
The AzSK docs for control attestation are [here](../00c-Addressing-Control-Failures/Readme.md#control-attestation-1)

#### Attestation fails with permission error even though I am Owner.

It can happen due to the below reasons
...
3. AzSKRG resource group is not present on the subscription </br>
   Sol. => Setting up Continuous Assurance on your subscription will create the required AzSK artifacts.
   ```PowerShell
...
There are many reasons why you may see 'new' control failures after you put in the effort to resolve the existing ones. Here are some common ones: </br>
a- You may have new resources that were created/deployed to the subscription. The first time CA scans these resources (usually within 24 hours of their creation), it will flag any security control failures. </br>
...
c- Baseline control set may change. As attacks get sophisticated, so must our defense. It is quite possible that a control that was not considered "core hygiene" might become so a few months later. This will mean that for existing resources, everyone now has a new control to fix. </br>
...
Phew...We hope we covered the main reasons. One last thought is that in the current "agile + DevOps + cloud" era, with all rapid iterations and changes happening at all layers, security has also become a much more 'continuous' effort compared to the past.
...
The control regarding ‘do not peer another vNet with ER-connected vNet’ is a precautionary one and we expect would apply to most typical users of ER-Networks. However, when there are advanced users/scenarios like the one described above, you can certainly ‘attest’ the control and tell the DevOps Kit to treat it as passed in the future. However, it assumes that the person/team doing so is doing the proper diligence in ensuring that the configuration is setup securely and that it would not lead to new pathways between the wild internet and the corporate environment.

The same thinking applies to many of the other network-infra related controls. For example, we have a control that flags off ‘IP forwarding’. This would apply to 99% of the scenarios…However, there are legit cases where the control may not apply and can be ‘attested’ (e.g., you may be doing additional traffic inspection using a virtual appliance on a network).
